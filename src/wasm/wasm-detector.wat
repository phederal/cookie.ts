(module
  ...
  ;; Быстрая детекция формата cookie с исправленной логикой
  (func $detectFormat (param $ptr i32) (param $len i32) (result i32)
    ...
    ;; Сканирование строки для подсчета структуры
    (block $scan_done
      (loop $scan_loop
        ...
        ;; Обработка whitespace и подсчет полей
    ...
    ;; Проверка на валидное имя cookie
    ...
    ;; Проверка на Netscape boolean pattern (простая проверка TRUE/FALSE)
    (if (i32.ge_u (local.get $fieldCount) (i32.const 4))
      (then
        ;; Ищем паттерн TRUE или FALSE в строке
        (local.set $i (i32.const 0))
        (block $bool_search_done
          (loop $bool_search_loop
            ...
            (if (i32.or
              ...
              (then
                ;; Проверяем что это начало слова
                (if (i32.or
    ...
    (block $check_done
      (loop $check_loop
        ...
        (if (i32.and (local.get $inSemicolon) (i32.eq (local.get $char) (i32.const 61)))
          (then
            ;; Находим начало слова перед =
            (local.set $wordStart (i32.sub (local.get $i) (i32.const 1)))
            (block $word_start_done
              (loop $word_start_loop
                (br_if $word_start_done (i32.lt_s (local.get $wordStart) (local.get $firstEqualPos)))

                (local.set $wordChar (i32.load8_u (i32.add (local.get $ptr) (local.get $wordStart))))
                (if (i32.or
                      (i32.eq (local.get $wordChar) (i32.const 32)) ;; ' '
                      (i32.eq (local.get $wordChar) (i32.const 59)) ;; ';'
                    )
                  (then
            ...
            ;; Проверяем известные атрибуты
            (local.set $isKnownAttr (i32.const 0))

            ;; Domain (6 символов)
            (if (i32.eq (local.get $wordLen) (i32.const 6))
              (then
                (if (i32.and
                      (i32.and
                        (i32.or
                          (i32.eq (i32.load8_u (i32.add (local.get $ptr) (local.get $wordStart))) (i32.const 68)) ;; 'D'
                          (i32.eq (i32.load8_u (i32.add (local.get $ptr) (local.get $wordStart))) (i32.const 100)) ;; 'd'
                        )
                        (i32.or
                          (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $wordStart) (i32.const 1)))) (i32.const 79)) ;; 'O'
                          (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $wordStart) (i32.const 1)))) (i32.const 111)) ;; 'o'
                        )
                      )
                      (i32.or
                        (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $wordStart) (i32.const 5)))) (i32.const 78)) ;; 'N'
                        (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $wordStart) (i32.const 5)))) (i32.const 110)) ;; 'n'
                      )
                    )
                  (then (local.set $isKnownAttr (i32.const 1)))
                )
              )
            )

            ;; Path (4 символа)
            (if (i32.eq (local.get $wordLen) (i32.const 4))
              (then
                (if (i32.and
                      (i32.or
                        (i32.eq (i32.load8_u (i32.add (local.get $ptr) (local.get $wordStart))) (i32.const 80)) ;; 'P'
                        (i32.eq (i32.load8_u (i32.add (local.get $ptr) (local.get $wordStart))) (i32.const 112)) ;; 'p'
                      )
                      (i32.or
                        (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $wordStart) (i32.const 3)))) (i32.const 72)) ;; 'H'
                        (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $wordStart) (i32.const 3)))) (i32.const 104)) ;; 'h'
                      )
                    )
                  (then (local.set $isKnownAttr (i32.const 1)))
                )
